...
The current evidence points to the second interpretation.

## Current Read

The pose-free weighted anisotropic bottleneck is real, but it does not look like a general threat to BGP.

Why:

- the forward and inverse evidence for the core control-object story is already strong in the symmetric two-source, asymmetry, hyperbola, anisotropy, and multi-source branches
...
Too much of the current inverse story still passes through the centroid-normalized radial-signature pipeline.
...
The current bottleneck is scientifically interesting and may still produce publishable mechanism insights.

But it now looks like a branch-specific disentanglement problem, not the highest-priority theory-hardening problem.
...
The result was the hoped-for one:

- the core inferential BGP story survived the representation swap
...
> the bottleneck is still important, but it does not currently look like the highest-priority threat to BGP. The next theory-hardening priority should move to operational usefulness and scope boundaries.
