...
The experiment script is [run_regime_router_experiment.py](/Users/velocityworks/IdeaProjects/shape-budget/concepts/shape-budget/run_regime_router_experiment.py#L1).
...
The result is informative, but not in the simplest hoped-for way.
...
The summary file is [regime_router_summary.json](/Users/velocityworks/IdeaProjects/shape-budget/concepts/shape-budget/regime_router_outputs/regime_router_summary.json).
...
This branch behaves in a way that partly supports the routing idea.
...
The cell summaries show why the scalar story is not complete.
...
- the bottleneck really may be a policy-routing problem rather than a single-estimator problem
- the raw ratio carries real signal in the sparse-full branch
- the current support-aware index is not yet the right compressed variable
...
- sparse-full may be a ratio-routable branch
- sparse-partial may need an explicit support-aware gate before any scalar alias measure becomes useful

## Figures

Key figures:

- [regime_router_scatter.png](/Users/velocityworks/IdeaProjects/shape-budget/concepts/shape-budget/regime_router_outputs/figures/regime_router_scatter.png)
- [regime_router_method_bars.png](/Users/velocityworks/IdeaProjects/shape-budget/concepts/shape-budget/regime_router_outputs/figures/regime_router_method_bars.png)
