...
- a candidate-conditioned local `shift + alpha` search has now been completed: it improves alpha recovery over observation-only shift marginalization in four of five regimes and strongly in the sparse-partial branch, but it does not uniformly rescue the moderate sparse band, which means the orbit-alias story is real but still incomplete
...
- whether the sparse-full moderate mid-skew failure is mainly a wrong-family selection problem, a deeper representation-level alias, or a limitation of within-family local alpha search
