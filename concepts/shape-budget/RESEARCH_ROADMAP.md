...
- whether orientation-locking or other symmetry-breaking pre-alignment can recover `alpha` much more than geometry
- how much of the remaining pose-free `alpha` loss disappears under oracle alignment versus practical alignment
...
| pending | Orientation-locking pre-alignment | Symmetry-breaking test | Success: `alpha` error moves strongly back toward canonical-pose performance while geometry changes only modestly |
| pending | Oracle alignment ceiling | Upper-bound test | Success: oracle alignment closes most of the pose-free `alpha` gap and shows how much headroom practical alignment still leaves |
| pending | Alignment failure map | Robustness map | Success: the regimes where locking helps and where it becomes ill-posed are clearly separated |

The original roadmap sequence is complete, and the post-roadmap multi-source robustness plus Euclidean and anisotropic inverse extensions are now complete as well. The experiments hardened the existing claim first and then broadened it in a controlled way, and they have now surfaced a specific bottleneck: under pose-free observation, the latent family broadens mainly along the medium-anisotropy direction rather than along normalized geometry. Naive invariant compression does not fix that, softer shift-marginalized pose handling does recover a real part of the lost signal, and a one-seed local cleanup pass is not enough to turn that tighter local organization into materially better `alpha` identification. The new orbit-proximity diagnostic supports the careful version of the symmetry-orbit story, but it also narrows it: `alpha` is more orbit-close than geometry in the current signature space, yet the clean full-signature effect is only moderate, so the next highest-leverage step is still symmetry-breaking alignment rather than treating orbit proximity as the whole explanation.
