...
- exactly where alignment becomes ill-posed across anisotropy strength, support fraction, and source geometry
...
The original roadmap sequence is complete, and the post-roadmap multi-source robustness plus Euclidean and anisotropic inverse extensions are now complete as well. The experiments hardened the existing claim first and then broadened it in a controlled way, and they have now surfaced a specific bottleneck: under pose-free observation, the latent family broadens mainly along the medium-anisotropy direction rather than along normalized geometry. Naive invariant compression does not fix that, softer shift-marginalized pose handling does recover a real part of the lost signal, and a one-seed local cleanup pass is not enough to turn that tighter local organization into materially better `alpha` identification. The orbit-proximity diagnostic supports the careful symmetry-orbit story, the direct orientation-locking intervention shows that naive symmetry-breaking is too brittle under incomplete support, and the oracle ceiling now shows that most of the lost `alpha` signal is in fact recoverable with perfect pose. That makes alignment stability, not latent impossibility, the main open problem.
