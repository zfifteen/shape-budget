...
The sufficiency result holds for the symmetric, isotropic, equal-weight case. Natural generalizations that break these symmetries and that may require a higher-dimensional control space include:

1. **Weighted asymmetry** — unequal budget splits or growth rates; the single ratio \(e\) may no longer be sufficient and an additional weight parameter may be required.
2. **Anisotropic media** — replacing Euclidean distance with a directional cost function; the locus is generally no longer an ellipse and \(e\) may need to be replaced by a richer directional descriptor.
3. **Multi-source generalization** — for \(n > 2\) sources with a shared budget constraint, the control space may require multiple normalized allocation variables rather than a single ratio. One natural candidate is an \((n-1)\)-simplex, with \(e\) as the special two-source case.
