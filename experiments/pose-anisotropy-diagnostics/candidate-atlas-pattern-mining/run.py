...
ATLAS_DIR = ROOT / "experiments/pose-anisotropy-diagnostics/candidate-atlas-instrumentation/outputs"
PRESSURE_DIR = ROOT / "experiments/pose-anisotropy-interventions/backbone-correction-pressure-triggered-alpha-solver/outputs"
...
PRESSURE_TRIALS_PATH = PRESSURE_DIR / "backbone_correction_pressure_triggered_alpha_solver_trials.csv"
...
def assign_archetype(row: pd.Series) -> str:
    ...
    if float(row["cluster_alpha_span"]) >= 0.35 and float(row["cluster_geometry_span"]) >= 0.12:
        return "broad_fan"
...
def choose_threshold(calibration: pd.DataFrame, metric: str) -> tuple[float, float, float]:
        ...
        rows.append(
            {
                ...
                "mean_dominant_core_mass": float(subset["dominant_core_mean_mass"].mean()),
                "mean_broad_fan_mass": float(subset["broad_fan_mean_mass"].mean()),
                "mean_alpha_fan_mass": float(subset["alpha_fan_mean_mass"].mean()),
                "mean_fringe_singleton_mass": float(subset["fringe_singleton_mean_mass"].mean()),
                "mean_compact_minor_mass": float(subset["compact_minor_mean_mass"].mean()),
                "mean_fan_vs_core": float(subset["fan_vs_core"].mean()),
                "mean_useful_structure_ratio": float(subset["useful_structure_ratio"].mean()),
                "mean_fringe_burden": float(subset["fringe_burden"].mean()),
                "mean_residual_shell_alpha_mass": float(subset["residual_shell_alpha_mass"].mean()),
...
def summarize_by_condition(trial_patterns: pd.DataFrame) -> list[dict[str, object]]:
    ...
    for split in BLOCK_SPECS:
        for condition in FOCUS_CONDITIONS:
            subset = trial_patterns[(trial_patterns["split"] == split) & (trial_patterns["condition"] == condition)]
            if subset.empty:
                continue
            rows.append(
                {
...
def plot_condition_masses(path: str, condition_summary: list[dict[str, object]]) -> None:
    ...
    ax.bar(x, fringe, bottom=core + broad + alpha, color="#6d597a", label="fringe_singleton")
    ax.bar(x, compact, bottom=core + broad + alpha + fringe, color="#84a59d", label="compact_minor")
    ax.set_xticks(x)
...
def main() -> None:
    ...
    bank_count = 5.0
    trial_archetypes = (
        clusters.pivot_table(index=KEY, columns="archetype", values="cluster_mass_layer1", aggfunc="sum", fill_value=0.0)
        .reset_index()
    ...
    for key_values, group in clusters.groupby(KEY):
        ...
        shell_metric_rows.append(
            {
                **key_dict,
                "residual_shell_mass": float(group.loc[group["cluster_mean_score_gap"] > 0.00055, "cluster_mass_layer1"].sum() / bank_count),
                "residual_shell_alpha_mass": float(
                    group.loc[
                        (group["cluster_mean_score_gap"] > 0.00055) & (group["cluster_alpha_span"] > 0.25),
                        "cluster_mass_layer1",
                    ].sum()
                    / bank_count
                ),
    ...
    for column in ["dominant_core", "broad_fan", "alpha_fan", "fringe_singleton", "compact_minor"]:
    ...
    trial_archetypes["fan_vs_core"] = trial_archetypes["broad_fan_mean_mass"] / np.maximum(
        trial_archetypes["broad_fan_mean_mass"] + trial_archetypes["dominant_core_mean_mass"],
        NUMERIC_EPS,
    )
    trial_archetypes["fringe_vs_fan"] = trial_archetypes["fringe_singleton_mean_mass"] / np.maximum(
        trial_archetypes["fringe_singleton_mean_mass"] + trial_archetypes["broad_fan_mean_mass"],
        NUMERIC_EPS,
    )
    trial_archetypes["useful_structure_ratio"] = (
        trial_archetypes["broad_fan_mean_mass"] + trial_archetypes["alpha_fan_mean_mass"]
    ) / np.maximum(
    ...
    trial_archetypes["fringe_burden"] = trial_archetypes["fringe_singleton_mean_mass"] + (
    ...
    trial_patterns = atlas_trials.merge(trial_archetypes, on=KEY, how="left").merge(layer3_subset, on=KEY, how="left")
    trial_patterns["improve"] = (
        trial_patterns["anchored_alpha_output_abs_error"] - trial_patterns["refined_alpha_output_abs_error"]
    )

    gate_open_patterns = trial_patterns[trial_patterns["gate_open_flag"] == 1].copy()
    calibration_open = gate_open_patterns[gate_open_patterns["split"] == "calibration"].copy()
    fan_threshold, fan_cal_error, fan_fire_rate = choose_threshold(calibration_open, "fan_vs_core")
        ...
        fan_rule_rows.append(
            {
                **row.to_dict(),
                ...
                "fan_rule_output_abs_error": float(
                    row["refined_alpha_output_abs_error"] if fire == 1 else row["anchored_alpha_output_abs_error"]
                ),
            }
        )
    ...
    cluster_mass_condition = (
        archetype_rows.groupby(["archetype", "condition"]).size().unstack(fill_value=0).reset_index()
    )

    old_poison_candidates = atlas_rows[(atlas_rows["in_band_flag"] == 1) & (atlas_rows["consensus_weight_layer1"] >= 0.02)].copy()
    ...
    old_poison_join = old_poison_candidates.merge(
        layer3_subset[KEY + ["anchored_alpha_output_abs_error", "refined_alpha_output_abs_error", "correction_pressure"]],
        on=KEY,
        how="left",
    )
    old_poison_join["improve"] = (
        old_poison_join["anchored_alpha_output_abs_error"] - old_poison_join["refined_alpha_output_abs_error"]
    )
    ...
    summary_payload = {
        ...
        "archetype_condition_counts": {
            ...
            for archetype, condition_counts in archetype_rows.groupby(["archetype", "condition"]).size().unstack(fill_value=0).to_dict("index").items()
        },
        "gate_open_correlations": {
            "fan_vs_core_vs_improve": safe_corr(gate_open_patterns["fan_vs_core"], gate_open_patterns["improve"]),
            "fan_vs_core_vs_correction_pressure": safe_corr(
                gate_open_patterns["fan_vs_core"], gate_open_patterns["correction_pressure"]
            ),
            "useful_structure_ratio_vs_improve": safe_corr(
                gate_open_patterns["useful_structure_ratio"], gate_open_patterns["improve"]
            ),
            "useful_structure_ratio_vs_refined_alpha_bank_log_span": safe_corr(
                gate_open_patterns["useful_structure_ratio"], gate_open_patterns["refined_alpha_bank_log_span"]
            ),
            "fringe_burden_vs_improve": safe_corr(gate_open_patterns["fringe_burden"], gate_open_patterns["improve"]),
            "fringe_burden_vs_correction_pressure": safe_corr(
                gate_open_patterns["fringe_burden"], gate_open_patterns["correction_pressure"]
            ),
            "residual_shell_alpha_mass_vs_improve": safe_corr(
                gate_open_patterns["residual_shell_alpha_mass"], gate_open_patterns["improve"]
            ),
            "residual_shell_alpha_mass_vs_correction_pressure": safe_corr(
                gate_open_patterns["residual_shell_alpha_mass"], gate_open_patterns["correction_pressure"]
            ),
        },
        "fan_vs_core_rule": {
            ...
            "holdout_error": float(
                mean_or_nan(
                    fan_rule_df.loc[fan_rule_df["split"] == "holdout", "fan_rule_output_abs_error"].astype(float).tolist()
                )
            ),
            "holdout_fire_rate": float(
                mean_or_nan(
                    fan_rule_df.loc[fan_rule_df["split"] == "holdout", "fan_rule_fire_flag"].astype(float).tolist()
                )
            ),
            "confirmation_error": float(
                mean_or_nan(
                    fan_rule_df.loc[fan_rule_df["split"] == "confirmation", "fan_rule_output_abs_error"].astype(float).tolist()
                )
            ),
            "confirmation_fire_rate": float(
                mean_or_nan(
                    fan_rule_df.loc[fan_rule_df["split"] == "confirmation", "fan_rule_fire_flag"].astype(float).tolist()
                )
            ),
        },
        "poison_heuristic_shift": {
            ...
            "old_candidate_poison_score_vs_correction_pressure": safe_corr(
                old_poison_join["old_poison_score"], old_poison_join["correction_pressure"]
            ),
    ...
    with open(os.path.join(OUTPUT_DIR, "candidate_atlas_pattern_summary.json"), "w", encoding="utf-8") as handle:
