...
- in a radial-signature boundary representation, the symmetric normalized family forms a one-dimensional curved manifold ordered by `e`
...
- a pilot direct-alpha-refinement pass under the same pose-free anisotropic setting improves fit and sometimes improves `alpha`, but it does not rescue `alpha` uniformly, which suggests the solver challenge is not only coarse search resolution
...
- a candidate-conditioned local `shift + alpha` search has now been completed: it improves alpha recovery over observation-only shift marginalization in four of five regimes and strongly in the sparse-partial branch, but it does not uniformly rescue the moderate sparse band and still fails sharply in the sparse-full moderate family, which means the orbit-alias mechanism is real but still incomplete
- a probe-specialization experiment has now been completed: under equal-budget dedicated sampling, the best practical inverse probe changes with depletion phase, mainly between width and perimeter rather than between perimeter and major-tip curvature; a simple perimeter-pilot router modestly beats the best fixed probe in three of four regimes, which gives BGP partial support as an experimental-control principle
...
- whether the pose-free `alpha` recovery challenge can be materially reduced by a better representation, uncertainty estimate, or joint local refinement beyond alpha-only search
...
The original roadmap sequence is complete, and the post-roadmap multi-source robustness plus Euclidean and anisotropic inverse extensions are complete as well. The experiments hardened the base claim first and then broadened it in a controlled way, and they have now surfaced a specific solver challenge: under pose-free observation, the latent family broadens mainly along the medium-anisotropy direction rather than along normalized geometry. Naive invariant compression does not fix that, softer shift-marginalized pose handling recovers a real part of the lost signal, and a one-seed local cleanup pass is not enough to turn that tighter local organization into materially better `alpha` identification. The orbit-proximity diagnostic establishes the symmetry-orbit mechanism, the direct orientation-locking intervention shows that naive symmetry-breaking is too brittle under incomplete support, the oracle ceiling shows that most of the lost `alpha` signal is recoverable with perfect pose, and the failure map shows that the remaining practical problem is concentrated in sparse or partial low-to-mid-skew regions where simple locking loses large oracle headroom. A newer candidate-conditioned local `shift + alpha` intervention showed that the orbit-alias mechanism is actionable but incomplete: local candidate-family disentanglement helps substantially in the sparse-partial branch and in several other hard cells, but it does not uniformly solve the moderate sparse band and still fails sharply in the sparse-full moderate family. The multi-seed family-switching refinement sharpened that diagnosis further: sparse-full moderate cells do benefit from geometry-level family switching, including the original mid-skew target slice, while sparse-partial moderate cells often get worse under the same added freedom. The dominant corrective motion is usually geometric rather than anisotropic. The first orbit-alias regime-router pilot then added one more useful constraint: a raw anisotropy-to-skew ratio nearly matches the right policy in the sparse-full branch, but neither that scalar nor the first support-aware alias index is enough to beat the fixed alpha-only policy overall, because the sparse-partial branch still resists one-scalar routing. The representation-independence test then clarified the bigger picture: the core inferential BGP result survives a swap from radial signatures to support-function profiles, including useful canonical recovery, strong anisotropy-aware advantage over a Euclidean baseline, and the selective pose-free `alpha` penalty. The newer probe-specialization test extended that theory-hardening work from representation to action: under equal-budget dedicated sampling, depletion phase changes which probe family is best, mainly between width and perimeter, and a simple pilot router recovers a measurable part of that gain. The new scope-boundary experiment then closed the theory-hardening loop: exact one-knob scope now has explicit tested boundaries, the universal-one-scalar version is falsified by direct wrong-compression controls, the broader positive branches remain compact under small corrected control objects, and the current hard limit is the selective pose-free anisotropy solver challenge rather than general loss of compact structure. The current highest-value open work is therefore targeted solver challenge cleanup or harder out-of-family scope tests, not basic ambiguity about whether BGP stays compact.
...
- Risk: known-source identifiability looks strong but the harder unknown-source inverse problem can be substantially less stable.
...
- Risk: extension work can blur together genuinely new phenomena with artifacts of symmetry or Euclidean distance.
