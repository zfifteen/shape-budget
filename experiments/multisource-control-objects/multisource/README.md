...
The experiment script is [run_multisource_experiment.py](run.py#L1).
...
The summary file is [multisource_summary.json](outputs/multisource_summary.json).
...
- the equilateral slice forms a near one-parameter subfamily
...
That is stronger than an informal comparison, but narrower than claiming that the simplex itself is the full parameter space.

## What This Changes

Before this experiment, the central higher-dimensional question was whether multi-source geometry was still organized by a compact budget object.

After this experiment, there is direct evidence for that result:
...
That is exactly the sort of structure that makes Shape Budget a research program rather than a one-off result.
...
- a broader normalized family that forms a low-dimensional, roughly three-parameter manifold

This experiment does not address:

- that the simplex loop alone determines the whole geometry
- that unequal source weights preserve the same dimensional result
...
- [multisource_family_gallery.png](outputs/figures/multisource_family_gallery.png)
- [multisource_scale_collapse.png](outputs/figures/multisource_scale_collapse.png)
- [multisource_dimension.png](outputs/figures/multisource_dimension.png)

The clearest figures are:

- [multisource_scale_collapse.png](outputs/figures/multisource_scale_collapse.png) for the exact collapse of both the normalized boundary and the simplex loop
- [multisource_dimension.png](outputs/figures/multisource_dimension.png) for the “equilateral slice is near one-parameter, random family is low-dimensional but not one-knob” result

## Artifacts

Data:

- [multisource_residuals.csv](outputs/multisource_residuals.csv)
- [multisource_scale_collapse.csv](outputs/multisource_scale_collapse.csv)
- [multisource_spectra.csv](outputs/multisource_spectra.csv)
- [multisource_random_parameters.csv](outputs/multisource_random_parameters.csv)
- [multisource_summary.json](outputs/multisource_summary.json)

Code:

- [run_multisource_experiment.py](run.py#L1)
