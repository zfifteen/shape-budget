## THE BUDGET GOVERNOR PRINCIPLE

The Budget Governor Principle is the latent control parameter `e = c / a` for the symmetric constant-sum two-source process.

In that regime, normalized shape is governed by one ratio: separation divided by total reach budget. If the focal half-separation is `c` and the semimajor axis is `a`, then the control parameter is

`e = c / a`

and the normalized transverse residue is
...
- under the symmetric constant-sum two-source process, `e = c / a` governs the normalized shape family
- normalized observables become functions of `e`
- boundary data can recover `e`
