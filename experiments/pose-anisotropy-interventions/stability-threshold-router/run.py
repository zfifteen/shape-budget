"""
Stability-threshold router with cached candidate tables and a frozen ridge score.

The workflow is:

1. generate one cache table per explicit block of bank / observation seeds
2. fit exactly one ridge score model on cached calibration blocks only
3. fit one frozen cell-aware threshold map on calibration blocks only
4. freeze the score model and thresholds before any holdout generation or evaluation
5. validate on the requested holdout and confirmation blocks
6. if the first holdout fails, rerun exactly one density fallback branch
"""
...
def chooser_model_path(variant_name: str) -> str:
    return os.path.join(MODEL_DIR, f"{variant_name}__frozen_threshold_router.json")
...
def decision_threshold_path(variant_name: str) -> str:
    return os.path.join(MODEL_DIR, f"{variant_name}__decision_thresholds.json")


def candidate_thresholds_for_cell(
    rows: list[dict[str, float | str]],
    condition: str,
    skew_bin: str,
    ...
    if not subset:
        return [0.0]
    predicted = sorted(predict_delta(row, model) for row in subset)
    quantile_positions = [0.0, 1.0 / 6.0, 2.0 / 6.0, 3.0 / 6.0, 4.0 / 6.0, 5.0 / 6.0, 1.0]
    candidates = {0.0}
    for q in quantile_positions:
        idx = min(int(round(q * (len(predicted) - 1))), len(predicted) - 1)
        candidates.add(float(round(predicted[idx], 6)))
    return sorted(candidates)


def chooser_error_with_thresholds(
    rows: list[dict[str, float | str]],
    model: dict[str, object],
    ...
    errors: list[float] = []
    for row in rows:
        threshold = float(thresholds[cell_name(str(row["condition"]), str(row["geometry_skew_bin"]))])
        choose_joint = predict_delta(row, model) < threshold
        prefix = "joint" if choose_joint else "support"
        errors.append(float(to_float(row[f"{prefix}_alpha_error"])))
    return mean(errors)
...
def fit_decision_thresholds(
    ...
    for condition in FOCUS_CONDITIONS:
        for skew_bin in GEOMETRY_SKEW_BIN_LABELS:
            cell_rows = [row for row in calibration_rows if row["condition"] == condition and row["geometry_skew_bin"] == skew_bin]
            if not cell_rows:
                thresholds[cell_name(condition, skew_bin)] = 0.0
                continue
            candidates = candidate_thresholds_for_cell(calibration_rows, condition, skew_bin, model)
            best_threshold = 0.0
            best_error = float("inf")
            candidate_log: list[dict[str, float]] = []
            for threshold in candidates:
                trial_thresholds = {**thresholds, cell_name(condition, skew_bin): float(threshold)}
                errors: list[float] = []
                for row in cell_rows:
                    choose_joint = predict_delta(row, model) < float(threshold)
                    prefix = "joint" if choose_joint else "support"
                    errors.append(float(to_float(row[f"{prefix}_alpha_error"])))
                cell_error = mean(errors)
                candidate_log.append({
                    "threshold": float(threshold),
                    "cell_alpha_error_mean": float(cell_error),
                })
                if cell_error + NUMERIC_EPS < best_error or (abs(cell_error - best_error) <= NUMERIC_EPS and abs(threshold) < abs(best_threshold)):
                    best_error = float(cell_error)
                    best_threshold = float(threshold)
            thresholds[cell_name(condition, skew_bin)] = float(best_threshold)
            search_log.append(
                {
                    "condition": condition,
                    "geometry_skew_bin": skew_bin,
                    "selected_threshold": float(best_threshold),
                    "selected_cell_alpha_error_mean": float(best_error),
                    "threshold_candidates": candidate_log,
                }
            )

    payload = {
        "solver_family": "stability_threshold_router",
    ...
    with open(decision_threshold_path(variant_name), "w", encoding="utf-8") as handle:
        json.dump(payload, handle, indent=2)
    return payload


def load_decision_thresholds(variant_name: str) -> dict[str, object]:
    with open(decision_threshold_path(variant_name), "r", encoding="utf-8") as handle:
...
def choose_from_cached_row(
    row: dict[str, float | str],
    model: dict[str, object],
    threshold_payload: dict[str, object],
) -> dict[str, float | str]:
    predicted_delta = predict_delta(row, model)
    threshold = float(threshold_payload["decision_thresholds"][cell_name(str(row["condition"]), str(row["geometry_skew_bin"]))])
    choose_joint = int(predicted_delta < threshold)
...
def evaluate_on_rows(
    ...
    predictions = [choose_from_cached_row(row, model, threshold_payload) for row in rows]
...
def fit_and_freeze_official_model(variant_name: str) -> tuple[dict[str, object], dict[str, object]]:
    ...
    threshold_payload = fit_decision_thresholds(train_rows, model, variant_name, train_blocks)
    return load_model_artifact(variant_name), threshold_payload
...
def smoke_check(variant_name: str) -> dict[str, object]:
    ...
    smoke_thresholds = fit_decision_thresholds(train_rows, smoke_model, variant_name, ["calibration_block_1"])
    predictions, summary = evaluate_on_rows(train_rows, smoke_model, smoke_thresholds, "smoke_calibration_block_1")
    ...
    save_report(
        ...
        {
            "variant": VARIANT_SPECS[variant_name].__dict__,
            "decision_threshold_path": decision_threshold_path(variant_name),
            "summary": summary,
        },
    )
    return summary
...
def fit_eval_from_existing_cache(
    ...
    model, threshold_payload = fit_and_freeze_official_model(variant_name)
    ...
    calibration_predictions, calibration_summary = evaluate_on_rows(calibration_rows, model, threshold_payload, "calibration")
    holdout_predictions, holdout_summary = evaluate_on_rows(holdout_rows, model, threshold_payload, "holdout_block_1")
...
def stability_threshold_solver(
    observed_signature: np.ndarray,
    mask: np.ndarray,
    condition: str,
    ...
    threshold = float(threshold_payload["decision_thresholds"][cell_name(condition, geometry_skew_bin)])
    choose_joint = predicted_delta < threshold
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stability-threshold focused solver workflow.")
