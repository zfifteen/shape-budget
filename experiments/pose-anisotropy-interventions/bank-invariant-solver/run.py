"""
Bank-invariant solver with differential features for the ridge chooser.

The bank-adaptive solver cleared holdout but failed fresh-bank confirmation.
The root cause is that absolute score features (support_score, joint_score,
support_entropy, joint_entropy, etc.) shift when the reference bank changes,
causing the frozen ridge chooser to misroute on fresh banks.

This experiment replaces those absolute features with bank-invariant
...
The evaluation ladder is identical:

1. cache table per block
2. fit one ridge chooser on calibration blocks only
3. freeze before holdout
4. validate on holdout and confirmation
5. one density fallback branch if needed
"""
...
# ---------------------------------------------------------------------------
# Upstream symbols (same as bank-adaptive solver)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Bank-invariant feature names
#
# All score-based features are differentials (joint minus support) rather
# than absolute values.  Parameter-based features are also differentials.
# The cell one-hot is retained for support-type conditioning.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Trial generation (same as bank-adaptive solver)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Bank-invariant feature construction
# ---------------------------------------------------------------------------
...
def build_feature_vector(row: dict[str, float | str]) -> np.ndarray:
    ...
    support_alpha = max(to_float(row["support_alpha"]), NUMERIC_EPS)
    joint_alpha = max(to_float(row["joint_alpha"]), NUMERIC_EPS)
    support_score = to_float(row["support_score"])
    joint_score = to_float(row["joint_score"])
    support_cv = to_float(row["support_cv_score"])
    joint_cv = to_float(row["joint_cv_score"])

    log_alpha_diff = math.log(joint_alpha) - math.log(support_alpha)
    t_diff = to_float(row["joint_t"]) - to_float(row["support_t"])
    rho_diff = to_float(row["joint_rho"]) - to_float(row["support_rho"])
    h_diff = to_float(row["joint_h"]) - to_float(row["support_h"])
    w1_diff = to_float(row["joint_w1"]) - to_float(row["support_w1"])
    w2_diff = to_float(row["joint_w2"]) - to_float(row["support_w2"])
    score_diff = joint_score - support_score
    entropy_diff = to_float(row["joint_entropy"]) - to_float(row["support_entropy"])
    cv_diff = joint_cv - support_cv
...
def save_model_artifact(
    ...
    payload = {
        "solver_family": "bank_invariant_ridge_chooser",
...
def choose_from_cached_row(row: dict[str, float | str], model: dict[str, object]) -> dict[str, float | str]:
    ...
    margin = float(model.get("confidence_margin", CONFIDENCE_MARGIN))
    choose_joint = int(predicted_delta < -margin)
...
def summarize_predictions(rows: list[dict[str, float | str]], label: str) -> dict[str, object]:
    ...
    if beats_support and beats_joint:
        plain_language = (
            f"The bank-invariant chooser beat both cached candidates on {label}: "
            f"support {support_mean:.4f}, joint {joint_mean:.4f}, chooser {chooser_mean:.4f}."
        )
        bgp_interpretation = "strengthens BGP"
    elif beats_support:
        plain_language = (
            f"The bank-invariant chooser beat the support-aware baseline but not the joint candidate on {label}: "
            f"support {support_mean:.4f}, joint {joint_mean:.4f}, chooser {chooser_mean:.4f}."
        )
        bgp_interpretation = "narrows the remaining solver bottleneck"
    elif beats_joint:
        plain_language = (
            f"The bank-invariant chooser beat the joint candidate but not the support-aware baseline on {label}: "
            f"support {support_mean:.4f}, joint {joint_mean:.4f}, chooser {chooser_mean:.4f}."
        )
        bgp_interpretation = "narrows the remaining solver bottleneck"
    else:
        plain_language = (
            f"The bank-invariant chooser did not beat either cached candidate on {label}: "
            f"support {support_mean:.4f}, joint {joint_mean:.4f}, chooser {chooser_mean:.4f}."
        )
        bgp_interpretation = "narrows the remaining solver bottleneck"
...
def run_full_plan(force: bool = False) -> dict[str, object]:
    ...
    if baseline_result["holdout_success"] and not baseline_result["working_solver"]:
        final_payload = {
            ...
            "final_plain_language_result": (
                "The bank-invariant chooser cleared holdout block 1 but failed fresh-bank confirmation without recalibration; "
    ...
    if not density_result["holdout_success"]:
        final_payload["final_plain_language_result"] = (
            "The bank-invariant chooser stayed bank-sensitive after the allowed density ablation; "
            "current bank-invariant routing is too bank-sensitive for a reliable solution under this solver family."
        )
        final_payload["final_bgp_interpretation"] = "narrows the remaining solver bottleneck"
    elif not density_result["working_solver"]:
        final_payload["final_plain_language_result"] = (
            "The bank-invariant chooser cleared holdout block 1 but failed fresh-bank confirmation without recalibration; "
            "current bank-invariant routing is still too bank-sensitive for a reliable solution under this solver family."
        )
        final_payload["final_bgp_interpretation"] = "narrows the remaining solver bottleneck"
...
def bank_invariant_solver(
    ...
    margin = float(chooser_model.get("confidence_margin", CONFIDENCE_MARGIN))
    choose_joint = predicted_delta < -margin
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bank-invariant focused solver workflow.")
